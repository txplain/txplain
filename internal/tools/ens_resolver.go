...
func (e *ENSResolver) GetAnnotationContext(ctx context.Context, baggage map[string]interface{}) *models.AnnotationContext {
	...
	// Get all addresses from transaction data for mapping
	allAddresses := e.extractAllAddresses(baggage)
	if len(allAddresses) == 0 {
		return annotationContext
	}

	// Create address mapping context for annotation generator
	// This provides the critical mapping from shortened display addresses to full addresses
	for fullAddress := range allAddresses {
		cleanAddr := e.cleanAddress(fullAddress)
		if cleanAddr == "" {
			continue
		...
		ensName := ""
		if ensNames != nil {
			ensName = ensNames[cleanAddr]
		}

		// Add address mapping context item
		...
		annotationContext.AddItem(models.AnnotationContextItem{
			...
			Metadata: map[string]interface{}{
				...
				"shortened_address": shortAddr,
				"ens_name":          ensName,
				"original_address":  fullAddress,
			},
		})

		// Also add by full address for reverse lookup
		annotationContext.AddItem(models.AnnotationContextItem{
			Type:        "address_mapping",
			Value:       cleanAddr, // Full address
			Name:        fmt.Sprintf("Full address: %s", cleanAddr),
			Description: fmt.Sprintf("Full address for %s", shortAddr),
			Metadata: map[string]interface{}{
				"full_address":      cleanAddr,
				"shortened_address": shortAddr,
				"ens_name":          ensName,
		...
		// If ENS name exists, add ENS-specific context
		if ensName != "" {
			annotationContext.AddItem(models.AnnotationContextItem{
				Type:        "ens",
				Value:       ensName,
				Name:        fmt.Sprintf("ENS: %s", ensName),
				Description: fmt.Sprintf("ENS name for address %s", shortAddr),
				Metadata: map[string]interface{}{
					"full_address":      cleanAddr,
					"shortened_address": shortAddr,
					"ens_name":          ensName,
